...
export const getSearchResult = (queryText: string, filter = 'all', order = 'desc') => async (dispatch: AppDispatch, getState: () => RootState) => {
  ...
  const { data, success } = await apiRequest({
    endpoint: `/search/issues?q=${queryText} user:${username} is:open type:issue ${filter !== 'all' ? `label:"${getStatusFilterText(filter)}"` : ''}&sort=created&order=${order}&per_page=10&page=${prevQueryText === queryText ? page : 1}&state=open`,
  })
  if (success) {
